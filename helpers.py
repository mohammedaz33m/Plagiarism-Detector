...
def get_answer_source_text(df, answer_filename):
    '''This function is to get the answer text and its associated source text.
       
       :param df: A dataframe with columns,
           'File', 'Task', 'Category', 'Class', 'Text', and 'Datatype'
       :param answer_filename: A filename for an answer text in the df, ex. 'g0pB_taskd.txt'
       :return: an answer text and its source text.
    '''
    # file name format is always ending with _taska(b/c/d/e).txt
    source_filename = 'orig' + answer_filename[-10:]
    
    # Get source text's the index number in the dataframe
    source_index_number = df[df['File'] == source_filename].index.values.astype(int)[0]
    
    # Get answer text's the index number in the dataframe
    answer_index_number = df[df['File'] == answer_filename].index.values.astype(int)[0]
    
    # Source Text
    source_text = df.loc[source_index_number,'Text']
    
    # Answer Text
    answer_text = df.loc[answer_index_number,'Text']
    
    return source_text,answer_text